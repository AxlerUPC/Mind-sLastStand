...
void AEnemigo::TakeDamageFromPlayer()
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red,
			FString::Printf(TEXT("%s recibió daño. Vida restante: %.0f"), *GetName(), Health));
	}

	UE_LOG(LogTemp, Warning, TEXT("Enemigo golpeado. Vida: %.0f"), Health);
...
void AEnemigo::Die()
{
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Orange,
			FString::Printf(TEXT("%s murió!"), *GetName()));
	}
